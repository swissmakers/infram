...
export const QuickAction = ({ isOpen, onClose }) => {
    ...
    const allItems = useMemo(() => [
        ...
        ...getSidebarNavigation(t, user?.role === "admin").map(nav => ({ id: `nav-${nav.path.slice(1)}`, type: "navigation", name: nav.title, icon: nav.icon, route: nav.path, path: t("common.quickAction.navigation") })),
    ...
    ], [servers, allSnippets, t, user?.role]);
